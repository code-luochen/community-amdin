      ...
      <div class="premium-header relative overflow-hidden mb-5!">
        ...
        <div class="header-content relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div class="header-text">
            <h1 class="page-title">订单监督</h1>
            <p class="page-subtitle">守护长者，随时掌握服务预约与状态</p>
          </div>
      ...
      <div class="content-card shadow-soft">
        ...
        <div class="table-wrapper">
          <el-table :data="tableData" v-loading="loading" class="premium-table" style="width: 100%">
            ...
            <el-table-column label="预约时间时间" min-width="180">
              <template #default="{ row }">
                <div class="time-cell text-xs"><el-icon class="mr-1"><Clock /></el-icon>{{ formatDate(row.serviceTime) }}</div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="家属评价" min-width="170" fixed="right">
              <template #default="{ row }">
                <el-rate v-if="row.evaluation" v-model="row.evaluation" disabled show-score text-color="#ff9900" class="mini-rate" />
                <span v-else class="text-slate-400 text-xs italic inline-block w-full text-center">暂未评价</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
